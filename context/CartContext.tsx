...
export type CartItem = {
  ...
  quantity: number;
  size: string;
  color: { name: string; hex: string, image: number };
  image: string;
};
...
export const CartProvider = ({ children }: { children: ReactNode }) => {
  ...
  const removeItem = (id: string, size: string, colorName: string) => {
    setItems(items.filter(
      item => !(item.id === id && item.size === size && item.color.name === colorName)
    ));
  };

  const updateQuantity = (id: string, size: string, colorName: string, quantity: number) => {
    setItems(items.map(item => {
      if (item.id === id && item.size === size && item.color.name === colorName) {
